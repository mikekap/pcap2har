...
class HttpRequest:
    ...
    httpVersion: str = ''
    url: str = ''
    headers: dict[str, list[str]] = field(default_factory=lambda: defaultdict(lambda: []))
    ...
    for layer, packet in (x for p in tqdm.tqdm(file) for x in unnest(p)):
        ...
        match str(packet.frame_info.get_field('p2p_dir')):
            ...
            case _:
                if conv_details[full_stream_id].remoteAddress:
                    direction = 'send' if str(packet.ip.dst) == conv_details[full_stream_id].remoteAddress else 'recv'
                else:
                    direction = 'send'
        ...
        if header := layer.get_field('header') or layer.get_field('headers_header'):
            ...
            if full_uri := layer.get_field('request_full_uri'):
                if isinstance(my_conv_details, HttpRequest):
                    my_conv_details.url = full_uri
            if method := layer.get_field('request_method') or layer.get_field('headers_method'):
                my_conv_details.method = method
            ...
            'entries': [
                {
                    ...
                    'request': {
                        ...
                        'postData': {
                            'mimeType': first(conv.request.headers.get('content-type', [])),
                            'encoding': 'base64',
                            'text': base64.b64encode(conv.request.body).decode('ascii'),
                        } if conv.request.body else None,
                    ...
                    'response': {
                        ...
                        'content': {
                            ...
                            **content_to_json(first(conv.response.headers.get('content-type', [])), conv.response.body)
                        },
                    },
                    ...
                    '_webSocketMessages': [
                        {
                            ...
                            'time': m.time,
                            'opcode': m.opcode,
                            'data': m.data.decode('utf-8') if m.opcode != 0x2 else base64.b64encode(m.data).decode('ascii'),
                        }
                        for m in conv.websocketMessages
                    ] if conv.websocketMessages else None,
                    'cache': {},
                    'timings': {
                        'blocked': 0,
                        'dns': 0,
                        'connect': 0,
                        'send': (conv.request.endTimestamp - conv.request.startTimestamp) * 1000.0,
                        'wait': (conv.response.startTimestamp - conv.request.endTimestamp) * 1000.0 if conv.response.startTimestamp else -1,
                        'receive': (conv.response.endTimestamp - conv.response.startTimestamp) * 1000.0 if conv.response.startTimestamp else -1,
                        'ssl': 0,
                    },
